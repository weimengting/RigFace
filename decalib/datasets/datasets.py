...
class TestData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.size is not None: # size = 256
        ...
        # self.resolution_inp = 224,目标图像大小
        DST_PTS = np.array([[0, 0], [0, self.resolution_inp - 1], [self.resolution_inp - 1, 0]])
        # 计算源图像变换到目标图像需要经过怎样的矩阵转换
        tform = estimate_transform('similarity', src_pts, DST_PTS)

        image = image / 255. # 0-1区间

        dst_image = warp(image, tform.inverse, output_shape=(self.resolution_inp, self.resolution_inp))
        dst_image = dst_image.transpose(2,0,1)

        return {'image': torch.tensor(dst_image).float(), # 只对面部区域进行了保留
                'imagename': imagename,
                'tform': torch.tensor(tform.params).float(),
                'original_image': torch.tensor(image.transpose(2, 0, 1)).float(),
                }
...
if __name__ == '__main__':
    testdata_source = datasets.TestData(
        source, iscrop=True, size=512, sort=True
    )